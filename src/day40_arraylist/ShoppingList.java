...
public class ShoppingList {
    public static void main(String[] args) {
        ...
        if(input.nextLine().equals("yes")){
            System.out.println("What items do you want to remove");
            ...
            if(Character.isDigit(removeItem.charAt(0))) { // checks the first character of remove item checks if it is a number
                ...
                shoppingList.remove(number-1); //converts the number to an index. If the user wants to remover the first item, they would give 1, so to remove the first index 1-1=0
            } else {
